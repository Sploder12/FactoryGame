...
public class Render extends Canvas implements Runnable {
	...
	public static final byte bodies = 1, shirts = 2, hairs = 2, hats = 1, faces = 5, items = 1, objects = 1;
	...
	public static BufferedImage[] BObjects = new BufferedImage[objects];
	public Image[] object = new Image[objects];
	...
	public Render(){
		...
		try{
			...
			Bhead = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\Head.png"));
			
			for(int i = 0; i < 1; i++){
				BBmenus[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Menus\\"+ i + ".png")); //loads tilesets
				menus[i] = BBmenus[i].getScaledInstance((int)(800*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			for(int i =0; i <  worldFloorTiles; i++){
				Bfloors[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\World\\floor\\f"+ i + ".png"));
				floor[i] = Bfloors[i].getScaledInstance((int)(32*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			for(int i =0; i <  items; i++){
				BItems[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Items\\item"+ i + ".png"));
				item[i] = BItems[i].getScaledInstance((int)(32*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			for(int i =0; i <  objects; i++){
				BObjects[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Objects\\object"+ i + ".png"));
				object[i] = BObjects[i].getScaledInstance((int)(32*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			for(byte i = 0; i < bodies; i++){
				Bbodi[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\body"+ i + ".png")); //loads tilesets
			...
			for(byte i = 0; i < shirts; i++){
				for(byte j = 0; j < bodies; j++){
					Bshirt[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\"+ j +"shirt"+ i + ".png")); //loads tilesets
					shirt[i] = Bshirt[i].getScaledInstance((int)(48*xScale), -1, Image.SCALE_SMOOTH);
				}
			}
			
			for(byte i = 0; i < hairs; i++){
				Bhair[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\hair"+ i + ".png")); //loads tilesets
				hair[i] = Bhair[i].getScaledInstance((int)(48*xScale), -1, Image.SCALE_SMOOTH);
			}
			for(byte i = 0; i < hats; i++){
				Bhats[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\hat"+ i + ".png")); //loads tilesets
				hat[i] = Bhats[i].getScaledInstance((int)(48*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			for(byte i = 0; i < faces; i++){
				Bfaces[i] = ImageIO.read(new File("FactoryFrenzy\\Resources\\Player\\face"+ i + ".png")); //loads tilesets
				face[i] = Bfaces[i].getScaledInstance((int)(48*xScale), -1, Image.SCALE_SMOOTH);
			}
			
			Main.user.LoadData("FactoryFrenzy\\Saves\\null.pdat");
		}catch(Exception e){
			e.printStackTrace();
		}
...
private void drawMainPlayer(Graphics g) {
	...
	g.drawImage(Main.user.bodyimg, (int)(halfWidth - 24*xScale), (int)(halfHeight - 48*xScale), this); //draws the body 
	g.drawImage(shirt[Main.user.shirt + (Main.user.body * shirts)], (int)(halfWidth - 24*xScale), (int)(halfHeight - 48*xScale), this); //draws the shirt ontop
	g.drawImage(Main.user.headimg,(int)(halfWidth - 22*xScale), (int)(halfHeight- 73*xScale), this);  //draws the head
	g.drawImage(Main.user.faceimg, (int)(halfWidth - 22*xScale), (int)(halfHeight - 73*xScale), this); //draws the face
	g.drawImage(Main.user.hairimg, (int)(halfWidth - 22*xScale), (int)(halfHeight - 73*xScale), this); //draws the hair on head
	g.drawImage(hat[Main.user.hat], (int)(halfWidth - 24*xScale), (int)(halfHeight - 64*xScale), this); //draws the hat on head
	g.setColor(transWhite);
	g.drawString(Main.user.name,(int)(halfWidth - (Main.user.name.length()*4)*xScale), (int)(halfHeight - 65*xScale)); //name
}
	...
	for(int x  = 0; x < 29; x++){ //Player can only see 25x18 area around them
		for(int y = 0; y < 22; y++){ //29x22 is used to prevent stuttering when moving
			try{
				...
				g.drawImage(floor[Main.world.worldScape[(x-14)+Main.user.tileX][(y-11)+Main.user.tileY].ordinal()],Math.round(((x-2)*32-(Main.user.x%32))*xScale), Math.round(((y-2)*32-(Main.user.y%32))*yScale),this);
			}catch (Exception e){	
				g.drawImage(floor[4], Math.round(((x-2)*32-(Main.user.x%32))*xScale), Math.round(((y-2)*32-(Main.user.y%32))*yScale),this);
			}
		}
	}
}
...
private void drawUI(Graphics g) {
	...
	for(int inv = 0; inv < 8; inv++){
		...
		g.drawImage(item[Main.user.inventory[inv][3].ordinal()], Math.round((inv*95+30)*xScale), Math.round(495*yScale),this);
		g.setColor(transWhite);
		if(Main.user.selectedInv == inv) {
			g.fillRect(Math.round((inv*95+30)*xScale), Math.round(495*yScale), Math.round(65*xScale), Math.round(65*yScale));
		}
...
private void drawAboveObjects(Graphics g) {
	for(int x = 0; x < 29; x++) {
		for(int y = 0; y < 12; y++) {
			try{
				//System.out.println(Main.user.x/32);
				ObjectTiles obj = Main.world.worldObjects[(x-14)+Main.user.tileX][(y-11)+Main.user.tileY];
				g.drawImage(object[obj.ordinal()-1],Math.round(((x-2)*32-(Main.user.x%32))+obj.getXOffset()*xScale), Math.round(((y-2)*32-(Main.user.y%32)+obj.getYOffset())*yScale),this);
			}catch (Exception e){	
...
private void drawBelowObjects(Graphics g) {
	for(int x = 0; x < 29; x++) {
		for(int y = 12; y < 22; y++) {
			try{
				//System.out.println(Main.user.x/32);
			...
			for(int x  = 0; x < 29; x++){ //This is for shading, it is done last
				for(int y = 0; y < 22; y++){ //29x22 is used to prevent uneven shading when moving
					try{
						...
						//Studders and acts dumb so disabled for now
						//g.fillRect(Math.round(((x-2)*32-(Main.user.x%32))*xScale), Math.round(((y-2)*32-(Main.user.y%32))*yScale), Math.round(32*xScale), Math.round(32*yScale));
					}catch(Exception e){
