...
public class Player0 {
	...
	public Items[][] inventory = new Items[8][4];
	public int[][] invenCount = new int[8][4];
	public byte selectedInv = 0;
	...
	public void LoadData(String file){
		...
		for(byte i = 0; i < 8; i++){
			for(byte j = 0; j < 4; j++){
				inventory[i][j] = Items.values()[json.getIntValueOfDict(inpt, json.locateStringEnd(inpt, "invI" + (i*4 + j)))];
				invenCount[i][j] = json.getIntValueOfDict(inpt, json.locateStringEnd(inpt, "invC" + (i*4 + j)));
			}
		}
	...
	public void LocalSaveData(){
		try{
	        ...
	        File file = new File("FactoryFrenzy\\Saves\\"+ name +".pdat");
	        ...
	        for(byte i = 0; i < 8; i++){
	        	for(byte j = 0; j < 4; j++){
	        		...
	        		if(inventory[i][j] != null) {
	        			out = inventory[i][j].ordinal();
	        		}
		        	output.append("invI" + (i*4 + j) +": " + out + ',');
		        	output.append("invC" + (i*4 + j) +": " + invenCount[i][j] + ',');
	        	}
	        }
